...
//clone the reset_log
const session_log = JSON.parse(JSON.stringify(reset_log));
...
function make_card(data){
    ...
    const card_header = make_element('div',{
        class:'internal-header',
        html:('')?'Internal Number: '+'':''
    });
    const buttons_wrapper = make_element('div',{
        class:'buttons-wrapper',
    });
}
function make_die(result){
    const die = make_element('div',{
        class:'dice',
        data:result,
        html: `
...
function make_roll_container(result){
    const roll = make_element('div',{
...
function process_rolls(user_input){
    ...
    if (typeof user_input === 'number') {
        let i = 0
        while (i < user_input){
            const die_one =roll_die();
            const die_two = roll_die();
            const die_total = die_one + die_two;
            const this_result = { 'one': die_one, 'two': die_two, 'total': die_total };
...
function process_results(total,col){
    ...
    while(!valid && col < 14){
        ...
        if ((cards[total][col].uq && session_log['unique_log'].indexOf(total+','+col) > -1) || session_log['results']['internal_type'][cards[total][col].nm]['exclude']) {
            col++
        } else {
            if (cards[total][col].uq) {
...
function internal_message(result){
    const name = result.nm;
    let internal_info = '<div class="internal-info">'+name+'</div> <div class="internal-info-additional">';
    if (name.toLowerCase().indexOf('hull') > -1) {
        if (name.toLowerCase().indexOf('aft') > -1) {
            internal_info += ' or Rear Hull';
        }
        internal_info += ' or Center Hull';
    } else if (name.toLowerCase().indexOf('apr') > -1) {
        internal_info += ' or AWR';
    } else if ((name.toLowerCase().indexOf('phaser') > -1 || name.toLowerCase().indexOf('torpedo') > -1 || name.toLowerCase().indexOf('drone') > -1) && (session_log['results']['internal_type'][name]['total'] !== 0 && parseInt(session_log['results']['internal_type'][name]['total']+1) % 3 === 0)) {
